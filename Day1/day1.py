...
import itertools # this is to be used on the second puzzle portion using three numbers to total 2020

# First Step: Open the data
numFile = open('nums.txt')

# Check that all data is available and yes!
#for line in numFile:
#    line = line.strip('\n')
#    print(line)
lines = numFile.read().splitlines()
nums = []
for line in lines:
...
for line in range(len(nums)):
    for li in itertools.combinations(nums, line):
        if(sum(li) == 2020):
            print(li)
            print(f'Product of tuple: {prod(li)}')
            count += 1
            break
        if count == 2:
            break
